...
void MyWindow::prosta(int startx, int starty, int koniecx, int koniecy,int r, int g, int b){
    double a, d;
    int pom;
    int i;
    if(abs(startx - koniecx) > abs(starty - koniecy)){
        ...
        for(i = startx; i <= koniecx; i++){
            pom = static_cast<int>(a * i + d) ;
            rysujPiksel(i,pom,r,g,b);
        }
    } else {
        if(starty > koniecy){
            pom = startx;
