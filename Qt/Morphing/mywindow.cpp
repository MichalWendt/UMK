    ...
    ui(new Ui::MyWindow)
{
    ...
    img = new QImage(":/turtle.jpg");
    copy = new QImage(":/turtle.jpg");
    img2 = new QImage(":/cebula.jpg");
    copy2 = new QImage(":/cebula.jpg");
...
void MyWindow::Morphing(int s, pair<int,int> A, pair<int,int> B, pair<int,int> C, pair<int,int> At, pair<int,int> Bt, pair<int,int> Ct){
    ...
    Xmin = Xmax = An.first;
    Ymin = Ymax = An.second;
    if(Bn.first < Xmin){Xmin = Bn.first;}
    if(Cn.first < Xmin){Xmin = Cn.first;}
    if(Bn.first > Xmax){Xmax = Bn.first;}
    if(Cn.first > Xmax){Xmax = Cn.first;}
    if(Bn.second < Ymin){Ymin = Bn.second;}
    if(Cn.second < Ymin){Ymin = Cn.second;}
    if(Bn.second > Ymax){Ymax = Bn.second;}
    if(Cn.second > Ymax){Ymax = Cn.second;}
    ...
    for(k = 1; k < podzial - 1; k++){
        ...
        for(i = 0; i < licznik - 1; i++){
            for(j = 0; j < licznik - 1; j++){
                ...
                Morphing(k,punkty[i][j],punkty[i][j+1],punkty[i+1][j+1],punkty2[i][j],punkty2[i][j+1],punkty2[i+1][j+1]);
                Morphing(k,punkty[i][j],punkty[i+1][j],punkty[i+1][j+1],punkty2[i][j],punkty2[i+1][j],punkty2[i+1][j+1]);
            }
        }
    }
    *img3 = wynik[0]->copy();
